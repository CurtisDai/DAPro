...
class UI:
    def __init__(self,window,window_name, show_small_window):
        window.title(window_name)
        self.show_small_window = show_small_window
        self.node_name = []
        self.canvas_node_id = {}
        self.node_position = {0:(550,120),1:(930,120),2:(230,400),
                              3:(1250,400),4:(550,680),5:(930,680)}
        self.color_list = ['lightgoldenrod','seashell','skyblue1','pink3',
                           'thistle1','palegreen','LightPink','HotPink','Plum',
                           'Purple','BlueViolet','LightSlateGray','LightSkyBlue',
                           'CadetBlue','MediumTurquoise','MediumSpringGreen','LawnGreen']
        self.edge_id = {}
        self.radius = 100
        self.tiny_radius = 20
        ...
        self.token_id = None
        self.img = tk.PhotoImage(file="key.gif")
    ...
    def add_edge(self, node0, node1, message):
        if node0!=node1 and (node0,node1) not in self.edge_id:
            self.color_index +=1
            ...
            if x0 != x1:
                ...
                angle = math.atan(-1 * float(y1 - y0) / float(x1 - x0)) * 180 / math.pi
                if x0 < x1:
                    ...
                    line = self.canvas.create_line(x0-radius, y0-delta-10, x1+radius, y1+delta-10,arrow=tk.LAST,fill=color, width=2)
                    text = self.canvas.create_text(x0-radius,y0-delta-10, text=message, width=200,fill=color,anchor=tk.SE, angle=angle)
            else:
                if y0 < y1:
                    line = self.canvas.create_line(x0-10, y0+radius, x1-10, y1-radius,arrow=tk.LAST,width=2,fill=color)
    ...
    def update_edge(self, first_node, second_node, target_message):
        if first_node != second_node and (first_node,second_node) in self.edge_id:
    ...
    def add_parent(self,node0, node1 ):
        if node0!=node1:
            ...
            if (node0, node1) in self.tiny_edge_id:
                self.canvas.delete(self.tiny_edge_id[(node0,node1)])
            ...
            if x0 != x1:
                if x0 < x1:
                    ...
                    line = self.canvas.create_line(x0+self.tiny_radius, y0, x1-self.tiny_radius, y1,arrow=tk.LAST, width=2)
     
                else:
                    line = self.canvas.create_line(x0-self.tiny_radius, y0, x1+self.tiny_radius, y1,arrow=tk.LAST, width=2)
            else:
                if y0 < y1:
                    line = self.canvas.create_line(x0, y0+self.tiny_radius, x1, y1-self.tiny_radius,arrow=tk.LAST,width=2)
                else:
                    line = self.canvas.create_line(x0, y0-self.tiny_radius, x1, y1+self.tiny_radius,arrow=tk.LAST,width=2)
            self.tiny_edge_id[node0,node1] = line

    def draw_token(self,node0):
        if self.token_id != None:
            self.canvas.delete(self.token_id)

        x0 = self.node_position[self.node_name.index(node0)][0]-80
        y0 = self.node_position[self.node_name.index(node0)][1]-80

        self.token_id = self.canvas.create_image(x0,y0, image=self.img)