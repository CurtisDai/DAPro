...
class UI:
    def __init__(self, window, window_name, show_small_window):
        ...
        self.node_position = {0: (550, 120),
                              ...
                              4: (550, 680),
                              5: (930, 680)}
        self.color_list = ['lightgoldenrod', 'seashell', 'skyblue1',
                           'thistle1', 'palegreen', 'Plum', 'Purple', 'BlueViolet', 'LightSlateGray', 'LightSkyBlue', 'CadetBlue',
        ...
        self.token_id = None
        self.img = tk.PhotoImage(file="key.png")
    ...
    def add_node(self, name):
        ...
        if self.show_small_window == True:
            ...
            if x0 != x1:
                ...
                if x0 < x1:
                    ...
                    text = self.canvas.create_text(x0 + radius, y0 + delta + 10, text=message, width=200, fill=color,
                                                   anchor=tk.NW, angle=angle)

                else:
                    line = self.canvas.create_line(x0 - radius, y0 - delta - 10, x1 + radius, y1 + delta - 10,
                                                   arrow=tk.LAST, fill=color, width=2)
                    text = self.canvas.create_text(x0 - radius, y0 - delta - 10, text=message, width=200, fill=color,
                                                   anchor=tk.SE, angle=angle)
            else:
                if y0 < y1:
                    line = self.canvas.create_line(x0 - 10, y0 + radius, x1 - 10, y1 - radius, arrow=tk.LAST, width=2,
                                                   fill=color)
                    text = self.canvas.create_text(x0 - 10, y0 + radius, text=message, width=200, fill=color,
                                                   anchor=tk.NW)
                else:
                    line = self.canvas.create_line(x0 + 10, y0 - radius, x1 + 10, y1 + radius, arrow=tk.LAST, width=2,
                                                   fill=color)
